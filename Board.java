...
public class Board {
    ...
    private boolean pinInBounds(int x, int y) {
        // can't have pins on the very edge?
    ...
    // might make this a method for note?
    private boolean noteContainsPoint(Note note, int x, int y) {
        return note.x <= x && x <= note.x + noteWidth && note.y <= y && y <= note.y + noteHeight;
    }
    ...
    public synchronized String get(String colorFilter, Integer containsX, Integer containsY, String refersToFilter) {
        // Check if contains filter coordinates are out of bounds
